...
Turn it into an encrypted vault, and run the app with the same env vars,
without editing a single line of application code. Budget: 60 seconds.
...
This downloads a single Go binary to `/usr/local/bin/tene`. No runtime
dependencies.
...
You will be prompted for a master password (twice). tene creates
...
Each KEY=VALUE line becomes an encrypted record. The values are not
written in plaintext anywhere during or after the import — the import
parses the file, encrypts each value in memory, writes ciphertext to the
vault, and discards the plaintext.
...
`tene run` spawns a subshell, sets each vault secret as an environment
...
Values are masked. You could run `tene get STRIPE_KEY` to see the actual
...
Your app code keeps using `process.env.KEY`, `os.Getenv("KEY")`,
...
gone from disk. AI coding agents cannot read what is no longer there.
