...
Eight commands. Per sprint. Nine times. The state machine inside `bkit` refuses to advance if a quality gate fails — typecheck, architecture violations, matrix-to-code sync, L1-L5 tests. I do not run `tsc --noEmit` myself anymore; the gate does. I do not measure architecture violations myself; `measure-arch-violations.ts` does, and the result is checked in as `arch-baseline.json` so the next sprint cannot regress without me being told.
