...
| `homebrew-tene` repo 생성 권한 (사용자가 직접 실행 필요) | 사용자 가이드: `gh repo create tene-ai/homebrew-tene --public --license MIT --add-readme` |
...
| Homebrew Formula 머지 지연 (외부 maintainer 리뷰) | 자체 tap (tene-ai/homebrew-tene) 우선; upstream Formula PR 은 v2.0 launch 후 |
