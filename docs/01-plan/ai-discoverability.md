...
| **S-1** Homebrew tap 활성화 | Infra | 30분 | 신규 리포 `tene-ai/homebrew-tap`, `HOMEBREW_TAP_GITHUB_TOKEN` 시크릿 | `brew install tene-ai/tap/tene && tene version` |
| **S-2** Docker GHCR 배포 | Infra | 45분 | `Dockerfile` 생성, `packages: write` 권한 | `docker run ghcr.io/tene-ai/tene version` |
...
- 신규 public 리포 `tene-ai/homebrew-tap` (비용 0)
...
- [ ] `brew install tene-ai/tap/tene` 실측 성공
- [ ] `docker run ghcr.io/tene-ai/tene version` 실측 성공
