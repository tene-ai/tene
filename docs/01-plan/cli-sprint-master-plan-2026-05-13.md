...
| Task | 상세 | AC |
|------|------|----|
| **T3-C1** | Homebrew tap 활성화: `gh repo create agent-kay-it/homebrew-tene --public --license MIT`; `HOMEBREW_TAP_GITHUB_TOKEN` 시크릿 설정; `.goreleaser.yml:109-140` 주석 해제 | `brew install agent-kay-it/tene/tene` 작동 |
...
| `homebrew-tene` repo 생성 권한 (사용자가 직접 실행 필요) | 사용자에게 명시적 가이드: `gh repo create agent-kay-it/homebrew-tene --public --license MIT --add-readme` |
...
| **T6-B7** | Homebrew tap merge + `brew install tene` 검증 (mac 사용자 5명에게 미리 베타 install 요청) | agent-kay-it/homebrew-tene |
